...
public class BluetoothLeIndependentService extends Service
{
	...
	public static final int IPC_COMMAND_PROGRAM_RESULT = 0x1002;
	public static final int IPC_COMMAND_PROGRAM_TASK_FINISH = 0x1003;
	...
	private final Runnable mRunnableTimeout = new Runnable()
	{
		...
		public void run()
		{
			...
			switch (mTimeoutTask)
			{
				case TASK_BOND:
				{
					appendLog("Connect timeout");
					LogUtil.d(TAG,"Connect timeout",Thread.currentThread().getStackTrace());
					broadcastNotifyUi(getConnectStatusIntent(CONNECTION_STATE_BOND_FAILED));
				}
				break;
				case TASK_BIND:
...
//	}
	...
	public void handleBleDisconnect()
	{
		try
		{
			...
			if(!mAutoConnectOnDisconnect || !mAllowReBind)
			{
