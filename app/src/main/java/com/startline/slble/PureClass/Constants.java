package com.startline.slble.PureClass;

/**
 * Created by terry on 2015/4/21.
 */
public class Constants
{
	public static final String CONFIG_FILE_SLBLE_SETTING = "FileSlbleSetting";
	public static final String CONFIG_ITEM_APP_SETTING = "ItemAppSetting";
	public static final String CONFIG_ITEM_BLE_SETTING = "ItemBleSetting";
	public static final String CONFIG_ITEM_AUTO_START_SETTING = "ItemAutoStartSetting";


	public static final String CONFIG_FILE_ACT_SETTING = "FileActSetting";
	public static final String CONFIG_ITEM_DEVICE_NAME = "ItemDeviceName";
	public static final String CONFIG_ITEM_DEVICE_ADDRESS = "ItemDeviceAddress";
	public static final String CONFIG_ITEM_WRITE_LOG = "ItemWriteLog";
}
